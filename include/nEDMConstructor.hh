//////////////////////////////////////////
// nEDMConstructor.hh //
// Base class to construct nEDM geometry  //
//////////////////////////////////////////

#ifndef nEDMConstructor_h
#define nEDMConstructor_h 1
...
private:
  ...
  nEDMDetectorConstruction *fConstruction;
  
  /// The parent of this constructor.
  nEDMConstructor *fParent;
  ...
  std::map<G4String,nEDMConstructor*> fSubConstructors;
...
public:
  nEDMConstructor(G4String name, nEDMDetectorConstruction *construct);
  nEDMConstructor(G4String name, nEDMConstructor *parent);
  virtual ~nEDMConstructor();
  ...
  nEDMDetectorConstruction *GetConstruction(void){
  ...
  void AddConstructor(nEDMConstructor* constructor){
    if(fSubConstructors.find(constructor->GetLocalName()) != fSubConstructors.end())
    {
      G4cout << "Multiply defined constructor name "
             << constructor->GetName()
             << G4endl;
      G4Exception("nEDMConstructor::AddConstructor()","Multiple defines",RunMustBeAborted,"");
    }
  ...
  template <class T> T& Get(G4String name){
    std::map<G4String,nEDMConstructor*>::iterator sub = fSubConstructors.find(name);
    if(sub == fSubConstructors.end())
    {
      ...
      G4Exception("nEDMConstructor::Get<>","Undefined constructor",RunMustBeAborted,"");
    }
    ...
    if(!constructor)
    {
      ...
      G4Exception("nEDMConstructor::Get<>","Bad typecast",RunMustBeAborted,"");
    }
    return *constructor;
  }
...
class nEDMConstructorMessenger: public G4UImessenger
{
private:
  nEDMConstructor* fConstructor;
...
public:
  nEDMConstructorMessenger(nEDMConstructor* constructor, const char* guide=NULL);
  virtual ~nEDMConstructorMessenger();
